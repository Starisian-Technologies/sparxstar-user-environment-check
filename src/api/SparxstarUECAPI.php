...
final class SparxstarUECAPI {
	 ...
	 */
	private const CLIENT_FIELDS_TO_EXTRACT = array(
	...
	public function handle_log_request( WP_REST_Request $request ): WP_REST_Response {
		...
		$full_snapshot = array_merge_recursive( $client_payload, array( 'server_side_data' => $server_data ) );
	...
	private function save_raw_snapshot_to_db( array $snapshot ): void {
		...
		$this->wpdb->insert(
			...
			array(
				...
				'server_side_data' => wp_json_encode( $snapshot['server_side_data'] ?? array() ),
				'client_side_data' => wp_json_encode( $snapshot['client_side_data'] ?? array() ),
	...
	public function get_latest_snapshot_from_db( ?int $user_id, ?string $session_id ): ?array {
		...
		$row['client_side_data'] = json_decode( $row['client_side_data'] ?? '[]', true ) ?? array();
		$row['server_side_data'] = json_decode( $row['server_side_data'] ?? '[]', true ) ?? array();
